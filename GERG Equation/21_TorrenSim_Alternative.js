...
function SetupFlowStreamSimulator(){
  //The following routine must be called once before any other routine.
    ...
    let Ei=zeros2(MaxFlds + 1, MaxFlds + 1);
    let Ki=zeros2(MaxFlds + 1, MaxFlds + 1);
    let Si=zeros2(MaxFlds + 1, MaxFlds + 1);
    let Wi=zeros2(MaxFlds + 1, MaxFlds + 1);
    let Bsnij;
    let Kij=zeros2(MaxFlds + 1, MaxFlds + 1);
    let Gij=zeros2(MaxFlds + 1, MaxFlds + 1);
    let Eij=zeros2(MaxFlds + 1, MaxFlds + 1);
    let Uij=zeros2(MaxFlds + 1, MaxFlds + 1);
    let n1;
    let n2;
    let T0;
    let d0;
...
class FlowStream {
  ...
  MolarMassDetail(){
    ...
    //    Mm - Molar mass (g/mol)
    this.MolarMass = 0;
    for(let i = 1 ; i <= NcDetail ; i++){this.MolarMass = this.MolarMass + this.x[i] * MMiDetail[i]}
  }
  CalculatePressure(){
    ...
    this.xTermsDetail();
    let ar = this.AlpharDetail(0);
    this.CompressibilityFactor = 1 + ar[0][1] / RDetail / this.Temperature;         //ar(0,1) is the first derivative of alpha(r) with respect to density
    this.Pressure = this.Density * RDetail * this.Temperature * this.CompressibilityFactor;
    dPdDsave = RDetail * this.Temperature + 2 * ar[0][1] + ar[0][2];  //d(P)/d(D) for use in density iteration
  }
  CalculateDensity(){
    ...
    let plog = Math.log(this.Pressure);
    let vlog = - Math.log(this.Density);
    let P2;
    let dpdlv;
    let vdiff;
    let tolr = 0.0000001;
    if(Math.abs(this.Pressure) < Epsilon){this.Density = 0; return}
    if (this.Density > -Epsilon){
      this.Density = this.Pressure / RDetail / this.Temperature;  //Ideal gas estimate
    }else{
    ...
    for(let it = 1; it <= 20; it++){
      if(vlog < -7 || vlog > 100){
        this.ierr = 1;
        this.herr = 'Calculation failed to converge in DETAIL method, ideal gas density returned.';
        this.Density = this.Pressure / RDetail / this.Temperature;
      }
  ...
  PropertiesDetail(){
    ...
    let RT;
    //
    this.MolarMassDetail();
    this.xTermsDetail();
    //Calculate the ideal gas Helmholtz energy, and its first and second derivatives with respect to temperature.
    let a0 = this.Alpha0Detail();
    //Calculate the real gas Helmholtz energy, and its derivatives with respect to temperature and/or density.
    let ar = this.AlpharDetail(2);
    RT = RDetail * this.Temperature;
    this.CompressibilityFactor = 1 + ar[0][1] / RT;
    this.Pressure = this.Density * RT * this.CompressibilityFactor;
    this.dPdD = RT + 2 * ar[0][1] + ar[0][2];
    this.dPdT = this.Density * RDetail + this.Density * ar[1][1];
    this.A = a0[0] + ar[0][0];
    this.S = -a0[1] - ar[1][0];
    this.U = this.A + this.Temperature * this.S;
    this.Cv = -(a0[2] + ar[2][0]);
    if (this.Density > Epsilon){
      this.H = this.U + this.Pressure / this.Density;
      this.G = this.A + this.Pressure / this.Density;
      this.Cp = this.Cv + this.Temperature * Math.pow((this.dPdT / this.Density), 2) / this.dPdD;
      this.d2PdD2 = (2 * ar[0][1] + 4 * ar[0][2] + ar[0][3]) / this.Density;
      this.JouleThomson = (this.Temperature / this.Density * this.dPdT / this.dPdD - 1) / this.Cp / this.Density;
    }else{
      this.H = this.U + RT;
      this.G = this.A + RT;
      this.Cp = this.Cv + RDetail;
      this.d2PdD2 = 0;
      this.JouleThomson = 1E+20;   //=(dB/dT*T-B)/Cp for an ideal gas, but dB/dT is not calculated here
    }
  ...
  xTermsDetail(){
    ...
    for(i = 1; i <= NcDetail; i ++){
      if(this.x[i] > 0){
        xi2 = this.x[i] * this.x[i];
        K3 = K3 + this.x[i] * Ki25[i];     //K, U, and G are the sums of a pure fluid contribution and a
        U = U + this.x[i] * Ei25[i];       //   binary pair contribution
        G = G + this.x[i] * Gi[i];
        Q = Q + this.x[i] * Qi[i];         //Q and F depend only on the pure fluid parts
        F = F + xi2 * Fi[i];
        for(n = 1; n <= 18; n++){
  ...
  // a0(2) - T*partial^2(a0)/partial(T)^2 [J/(mol-K)]
    ...
    for(i = 1; i <= NcDetail; i++){
      if(this.x[i] > 0){
        ...
        a0[1] = a0[1] + this.x[i] * (LogxD + n0i[i][1] - n0i[i][3] * (1 + LogT) + SumHyp1);
        a0[2] = a0[2] - this.x[i] * (n0i[i][3] + SumHyp2);
      }
    }
    a0[0] = a0[0] * RDetail * this.Temperature;
    a0[1] = a0[1] * RDetail;
    a0[2] = a0[2] * RDetail;
    return a0;
  }
  AlpharDetail(itau){
    ...
    let s2 = 0;
    let s3 = 0;
    let RT = RDetail * this.Temperature;
    ...
    let ar = Array(3).fill(0);
    ar[0] = Array(4).fill(0);
    ar[1] = Array(2).fill(0);
    ar[2] = Array(1).fill(0);
    if (Math.abs(this.Temperature - Told) > 0.0000001){
      for(n = 1; n <= 58; n++){
        Tun[n] = Math.pow(this.Temperature , -un[n]);
      }
    }
    Told = this.Temperature;
    //Precalculation of common powers and exponents of density
    Dred = K3 * this.Density;
    Dknn[0] = 1;
    for(n = 1; n <= 9; n++){

...
function CalculateMethaneNumberMWM(SimplifiedChromatografy){
    ...
    CalculateHowManyTimesIsTheComponentRepresented(HowManyComponentsAreRepresentedInThisTernary, AffinitiesOfEachTernary, IsThisComponentPresentHotOnes, HowManyTimesIsTheComponentRepresented, WillWeBeUsingThisTernaryHotOnes, NAji, VAji);
    //
    let RangeMinMaxAvgValueOfTheResult=Array(5).fill(0);
    let ActualMinimumRangeOfTheResultAchieved;
    ...
    for(let x = 0; x<=10000; x++){
        CalculateVAji(IsThisComponentPresentInThisTernaryHotOnes, SimplifiedChromatografy, VAji, SumOfNAjiComponentsInTheTernary, NAji, x, MinimumNAji);
        if(IsThisCompositionInsideBoundarys[VAji]){
            CalculateMethaneNumber(WillWeBeUsingThisTernaryHotOnes, VAji, CalculatedMethaneNumbers, RangeMinMaxAvgValueOfTheResult);
...
function CalculateMethaneNumber(WillWeBeUsingThisTernaryHotOnes, VAji, CalculatedMethaneNumbers, RangeMinMaxAvgValueOfTheResult){
    ...
    for(let i = 0; i<=18; i++){
        if(WillWeBeUsingThisTernaryHotOnes[i]){
            ...
            CalculatedMethaneNumbers[CalculatedMethaneNumbers.length-1] = FunctionA3(i, VAji);
        }
    }
    //This value is important. Is going to be the Objective Function
    RangeMinMaxAvgValueOfTheResult[2] = Math.min(CalculatedMethaneNumbers);
    RangeMinMaxAvgValueOfTheResult[3] = Math.max(CalculatedMethaneNumbers);
...
function CalculateVAji(IsThisComponentPresentInThisTernaryHotOnes, SimplifiedChromatografy, VAji, SumOfNAjiComponentsInTheTernary, NAji, x, MinimumNAji){
...
function CalculateHowManyTimesIsTheComponentRepresented(HowManyComponentsAreRepresentedInThisTernary, AffinitiesOfEachTernary, IsThisComponentPresentHotOnes, HowManyTimesIsTheComponentRepresented, WillWeBeUsingThisTernaryHotOnes, NAji, VAji){
...
function IsThisCompositionInsideBoundarys(VAji){
...
function FunctionA3(t, VAji){
    let Aux;
    for(let i = 0; i<=7; i++){
        for(let j = 0; j<=6; j++){
            Aux = Aux + a[t][i][j] * VAji[xyzOfTernary[t][1], t] ^ i * VAji[xyzOfTernary[t][2], t] ^ j;
        }
    }
    return Aux;
}
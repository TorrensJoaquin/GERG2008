...
function DensityGERG(iFlag,T,P,x){
//Sub DensityGERG(iFlag, T, P, x, D, ierr, herr)
...
let herr;
  ...
  //If requested, check to see if point is possibly 2-phase
    if (iFlag > 0) {
      [ Mm, PP, Z, dPdD, d2PdD2, d2PdTD, dPdT, U, H, S, Cv, Cp, W, G, JT, Kappa, A]=PropertiesGERG(T, D, x);
...
function ReducingParametersGERG(x, Tr, Dr){
...
function AlpharGERG(itau,idel,T,D,x){
...
// ar(2,0) -     tau^2*partial^2(ar)/partial(tau)^2
  ...
  [Tr, Dr]=ReducingParametersGERG(x, Tr, Dr);
  del = D / Dr;
  tau = Tr / T;
  lntau = Math.log(tau);
  delp[1] = del;
...
//If temperature has changed, calculate temperature dependent parts
  if (Math.abs(T - Told) > 0.0000001 || Math.abs(Tr - Trold2) > 0.0000001){tTermsGERG(lntau, x)}
  Told = T;
  Trold2 = Tr;
...
function PseudoCriticalPointGERG(x){
